...
from utils.dataset import MemoryDataset, MultipleDataset, IdxDataset
from sklearn.linear_model import SGDClassifier, LogisticRegression
from sklearn.neighbors import NearestNeighbors
import byol.builder_vit
import byol.loader
...
from trainer.contrast_trainer_byol import ContrastTrainerBYOL
...
parser.add_argument('-a', '--arch', metavar='ARCH', default='vit_small',
                    ...
                    help='model architecture: ' +
                        # ' | '.join(model_names) +
                        ' (default: vit_small)')
parser.add_argument('--weights', default='vit_small_patch16_224.augreg_in21k_ft_in1k',
                    help='timm models to load, this does not only decides the pretrained param, but also the net structure.')
parser.add_argument('--scratch', action='store_true',
                    help='whether train the vit model from scratch, or load the timm pretrained ckpt')
...
parser.add_argument('--wd', '--weight-decay', default=1e-1, type=float,
                    ...
                    dest='weight_decay')
parser.add_argument('--lora_wd', default=1e-1, type=float, help='lora weight decay (default: 1e-1)')
...
parser.add_argument('--hidden_size', default=512, type=int,
                    help='hidden dimension of all mlps (default: 512)')
parser.add_argument('--byol_momentum', default=0.996, type=float)
...
parser.add_argument('--weight_types', default='qv', type=str,
                    help='the layers of Vit that would attach lora, could be qv, qkvo, qkvom')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    base_param_list, lora_param_list = lora.separate_param(model=model.module.encoder_q)

    optimizer = torch.optim.AdamW(base_param_list, init_lr,
                                # momentum=args.momentum,
                                weight_decay=args.weight_decay)
    
    optimizer_lora = None
    if args.enable_lora:
        optimizer_lora = torch.optim.AdamW(lora_param_list , init_lora_lr,
                            # momentum=args.momentum,
    ...
    augmentation = [
        ...
        transforms.RandomApply([byol.loader.GaussianBlur([0.1, 2.0])], p=0.5),
    ...
    if args.enable_aug:
        train_dataset = datasets.ImageFolder(traindir, byol.loader.TwoCropsTransform(transforms.Compose(augmentation)))
    else:
    ...
    valid(small_train_loader, val_iid_loader, val_ood_loader, model.module.encoder_q, None, args.start_epoch, args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, None, optimizer, optimizer_lora, epoch, args)
        
        

        if epoch % args.save_freq == args.save_freq-1:
            if epoch == 199 or epoch == 249:
                full_valid(small_train_loader, val_iid_loader, val_ood_loader, model.module.encoder_q, None, epoch, args)
            else: 
                valid(small_train_loader, val_iid_loader, val_ood_loader, model.module.encoder_q, None, epoch, args)
...
def valid(train_loader, iid_loader, ood_loader, model, criterion, epoch, args):
    ...
    for ind, (x, y) in tqdm(enumerate(train_loader)):
        with torch.no_grad():
            ...
            features = model.forward_head(features, pre_logits=True).contiguous()
...
def train(train_loader, model, criterion, optimizer, optimizer_lora, epoch, args):
    ...
    contrast_step = ContrastTrainerBYOL(model)
    
    avg_meters = [batch_time, data_time, losses] if not args.enable_lora else [batch_time, data_time, losses, lora_absolute, lora_loss_meter]
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        if args.enable_lora:
            lora.lora_switch(model.module.encoder_q, use_lora=True)

        loss = contrast_step.run(images[0], images[1])
