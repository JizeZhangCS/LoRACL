...
# parser.add_argument('--lora_always_q', action='store_true',
                    # help='lora only and always set on encoder q (rather than switch from and to encoder & momentum encoder)')
...
parser.add_argument(
    "--moco-t", default=0.2, type=float, help="softmax temperature (default: 0.2)"
)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if len(weights)==0:
        weights = None
    model = moco.builder_vit.MoCo(
        # models.__dict__[args.arch],
        # timm.create_model(weights, pretrained=True),
        args.moco_dim,
        args.moco_k,
        args.moco_m,
        args.moco_t,
        ...
        if os.path.isfile(args.resume):
            ...
            if args.enable_lora:
                try:
                    ...
                    print("WARNING: optimizer_lora state_dict not found")
    ...
    if args.enable_aug:
        train_dataset = IdxDataset(traindir, moco.loader.TwoCropsTransform(transforms.Compose(augmentation)))
    else:
        train_dataset = IdxDataset(traindir, transform_plain)
...
def train(train_loader, model, criterion, optimizer, optimizer_lora, epoch, args):
    ...
    # enable_q_lora = True
    for i, (images, _, sample_idx) in enumerate(train_loader):
        # measure data loading time
        data_time.update(time.time() - end)

        sample_idx = sample_idx.cuda(args.gpu, non_blocking=True)
        if args.gpu is not None:
            if args.enable_aug:
        ...
        # lora.lora_switch(model.module.encoder_k, use_lora=not enable_q_lora)
        # if not args.lora_always_q:
            # enable_q_lora = not enable_q_lora

        loss = contrast_step.run(im_q=images[0], im_k=images[1], sample_idx=sample_idx)
