...
from trainer.contrast_trainer import ContrastTrainer
...
parser.add_argument('--pred-dim-ratio', default=4, type=int,
                    help='hidden dimension of the predictor / feature dimension (default: 4)')
...
parser.add_argument('--load_base_only', action='store_true',
                    help='only load base param when resume')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if len(weights)==0:
        weights = None
    model = simsiam.builder_vit.SimSiam(
        # models.__dict__[args.arch],
        args.dim, args.dim // args.pred_dim_ratio, weights, id_predictor=False, args=args)
    ...
    model.module.lora_init()
    base_param_list, lora_param_list = lora.separate_param(model=model.module.encoder)
    if args.fix_pred_lr:
        optim_params = [{'params': base_param_list, 'fix_lr': False},
    ...
    # full_valid(small_train_loader, val_iid_loader, val_ood_loader, model.module.encoder, criterion, args.start_epoch, args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % args.save_freq == args.save_freq-1:
            if epoch == 199 or epoch == 249:
                full_valid(small_train_loader, val_iid_loader, val_ood_loader, model.module.encoder, criterion, epoch, args)
            else: 
                valid(small_train_loader, val_iid_loader, val_ood_loader, model.module.encoder, criterion, epoch, args)
...
def train(train_loader, model, criterion, optimizer, optimizer_lora, epoch, args):
    ...
    contrast_step = ContrastTrainer(criterion, model, args)
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        if args.enable_lora:
            lora.lora_switch(model, use_lora=True)       
        ...
        if args.enable_lora:
            ...
            with torch.no_grad():
                ...
                lora_abs = _collect_lora_grad(model.module.encoder)
